...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    #(bs, num_attention_heads, seq_len, seq_len) @ (bs, num_attention_heads, seq_len, attention_head_size) --> (bs, num_attention_heads, seq_len, attention_head_size)
    y = att @ value
    # next, we need to concat multi-heads and recover the original shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
    concat_y = y.transpose(1, 2).contiguous().view(bs, seq_len, self.attention_head_size * self.num_attention_heads)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    out = self.self_attention.forward(hidden_states, attention_mask) #step 1
    x = self.add_norm(hidden_states, out, self.attention_dense, self.attention_dropout, self.attention_layer_norm) #step 2
    out2 = self.interm_af(self.interm_dense(x)) #step 3
    x2 = self.add_norm(x, out2, self.out_dense, self.out_dropout, self.out_layer_norm) #step 4
    return x2
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    embed = self.embed_dropout(self.embed_layer_norm(sum_embeds))
    ### TODO
